...
#         search_query = f"""
#             SELECT id,title, summary, link, image_url AS category_image, category, timestamp, sentiment,question
#             FROM articles
...
#     else:
#         cursor.execute('''
#             SELECT title, summary, link, image_url AS category_image, category, timestamp, sentiment,question
#             FROM articles
#             ORDER BY timestamp DESC
#         ''')
...
#                           recipients=['admin@example.com'],  # Change to your admin email
#                           body=body)
...
#         logged_in = False
#     cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
#     cursor.execute('SELECT * FROM articles WHERE link = %s', (article_link,))
#     article = cursor.fetchone()

#     if not article:
#         return "Article not found", 404

#     # Fetch related articles using raw SQL (since you're using MySQL directly)
#     cursor.execute('SELECT * FROM articles WHERE category = %s AND link != %s LIMIT 3', 
#                    (article['category'], article_link))
#     related_articles = cursor.fetchall()

#     return render_template('full_summary.html', article=article, related_articles=related_articles, logged_in=logged_in)
...
#     cursor.execute(
#         'SELECT * FROM articles WHERE category = %s AND id != %s LIMIT 3',
...
#         mysql.connection.commit()
#         # Track view
#     # Step 1: Get users who viewed this article
#     cursor.execute('SELECT DISTINCT user_id FROM views WHERE article_id = %s', (article_id,))
#     user_ids = [row['user_id'] for row in cursor.fetchall()]

#     related_articles = []

#     # Step 2: Get other articles viewed by those users
#     if user_ids:
#         format_ids = ','.join(['%s'] * len(user_ids))
#         query = f"""
#             SELECT a.*, COUNT(*) as view_count
#             FROM views v
#             JOIN articles a ON v.article_id = a.id
#             WHERE v.user_id IN ({format_ids}) AND v.article_id != %s
#             GROUP BY a.id
#             ORDER BY view_count DESC
#             LIMIT 3
#         """
#         cursor.execute(query, (*user_ids, article_id))
#         related_articles = cursor.fetchall()

#     # Step 3: Fallback if no articles found
...
#     # Track view and get user preferences
#     if user_id:
#         # Insert view record (ignore if already exists optionally)
#         cursor.execute('INSERT INTO views (user_id, article_id) VALUES (%s, %s)', (user_id, article_id))
#         mysql.connection.commit()

#         # Fetch preferences
#         cursor.execute('SELECT preferences FROM accounts WHERE id = %s', (user_id,))
#         pref_row = cursor.fetchone()
#         if pref_row and pref_row['preferences']:
#             preferences = [p.strip() for p in pref_row['preferences'].split(',')]
...
#     # Step 3: Fallback using preferences if no related articles found
#     if not related_articles and preferences:
#         format_prefs = ','.join(['%s'] * len(preferences))
#         query = f"""
#             SELECT * FROM articles
#             WHERE category IN ({format_prefs})
#             AND id != %s
#             LIMIT 3
#         """
#         cursor.execute(query, (*preferences, article_id))
#         related_articles = cursor.fetchall()

#     # Optional fallback: category match if still no articles
#     if not related_articles:
#         cursor.execute(
#             'SELECT * FROM articles WHERE category = %s AND id != %s LIMIT 3',
#             (article['category'], article_id)
